-- 원팀 행사(원데이 클래스, 6조) 선호도 조사 — index.html 이 쓰는 표.
-- 전용 Supabase 프로젝트(oneteam-vote)의 SQL Editor에 통째로 붙여넣어 실행한다.
-- 여러 번 실행해도 안전하다.
--
-- 로그인 없이 QR로 들어와 투표하는 페이지라 anon 키로 읽고 쓴다. 그래서
--   - 이름은 조원 명단 안에서만 받는다(check). 아무 이름이나 쌓이지 않게.
--   - 한 사람당 한 줄(primary key). 다시 투표하면 덮어쓴다.
-- 남의 이름으로 덮어쓰는 것까지 막지는 않는다. 10여 명 사내 설문에 그 정도 장치는 과하다.
